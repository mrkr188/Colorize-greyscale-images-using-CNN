...
# store all filenames
filenames = sorted(glob("Naruto/*.jpg"))
...
# Using this to print cost during training iteration
else:
...
try:
    while not coord.should_stop():
        ...
        # for every 10 iterations return the loss
        if step % 1 == 0:
            ...
            # for every 10 iterations return the summary image containing greyscale, predicted and true color image 
            summary_image = concat_images(grayscale_rgb_[0], pred_rgb_[0])
            summary_image = concat_images(summary_image, colorimage_[0])
            scipy.misc.imsave("summary2/" + "summary_" + str(step/10) + "_0.jpg", summary_image)

            sys.stdout.flush()

        # for every 10000 iterations save the model weights
        if step % 10000 == 9998:
            save_path = saver.save(sess, "full_model/colorize_model_2.ckpt")
